...
from pyotc.otc_backend.policy_iteration.entropic import entropic_otc
from pyotc.otc_backend.policy_iteration.entropic import entropic_otc1
...
def test_sbm_entropic_otc(transition, cost, entropic_cost):
    # entropic otc
    start = time.time()
    exp_cost, _, _ = entropic_otc(transition["P1"], transition["P2"], cost, xi=10, sink_iter=100)
    end = time.time()
...
def test_wheel_exact_otc():
    # entropic otc
    start = time.time()
    exp_cost12, _, _ = entropic_otc(P1, P2, c12, get_sd = True, L = 25, T = 50, xi = 0.1, sink_iter = 10)
    exp_cost13, _, _ = entropic_otc(P1, P3, c13, get_sd = True, L = 25, T = 50, xi = 0.1, sink_iter = 10)
    end = time.time()
    print(f"`entropic_otc` run time: {end - start}")
